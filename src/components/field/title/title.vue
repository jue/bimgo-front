...
const { setSelectedCell } = useTaskStore()
...
async function addNew(data) {
  ...
  if (!props.data.children) {
    props.data.children = []
    props.data.children.push({ title: '', parent_gid: data.gid, gid: null })
  }
  else {
    props.data.children.push({ title: '', parent_gid: data.gid, gid: null })
  }
  ...
  <div class="h-full w-full flex items-center min-h-[32px]">
    <div v-if="!isEdit" class="flex items-center w-full">
      ...
      <div class="items-center space-x-2 hidden btns">
        <span v-if="cate === 'task'" v-tooltip.bottom="'添加子任务'" class="cursor-pointer bg-gray-300 hover:bg-gray-500 w-4 h-4 rounded flex items-center justify-center" @click="addNew(data)">
