...
function deleteTask(data) {
  ElMessageBox.confirm(
    ...
    .then(async () => {
      ...
      if (res.code === 200)
        props.subTask.splice(props.subTask.findIndex(item => item.gid === data.gid), 1)
    })
    .catch(() => {
    })
}
...
<template>
  <div class="max-w-lg w-full space-y-3">
    <div v-if="subTask.length" class="space-y-1">
      <div v-for="item in subTask" :key="item.gid" class="flex items-center space-x-2">
        <TaskColumnStatus v-model="item.status" :row="item" />
        <div class="flex-1 px-2 min-h-8 rounded-md flex items-center cursor-pointer hover:bg-gray-100" @click="emit('to', item.gid)">
          {{ item.title }}
        </div>
        <div class="flex items-center space-x-2 text-gray-400">
          ...
          <el-dropdown trigger="click">
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item @click="emit('to', item.gid)">
