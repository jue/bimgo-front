...
export function useTableDragSort() {
  let callback
  function initDragSort(tableBodyDom, tableData) {
    setTimeout(() => {
      const trList = tableBodyDom.querySelectorAll('tr')
      const trHeight = trList[0].clientHeight
      const dragEnterTextList = tableBodyDom.querySelectorAll('tr .dragEnterText')
      const moveIconDoms = tableBodyDom.querySelectorAll('.moveIcon')
      ...
      for (let i = 0; i < trList.length; i++) {
        ...
        // 进入tr，展示高亮线
        trList[i].ondragenter = (e) => {
          if (isDragInChildNode(flatData[currentDragIndex], flatData[i])) {
            lightLine.style.cssText += ';display: none'
            return
          }
          const dragEnterText = trList[i].querySelector('.dragEnterText')
          const parentDom = dragEnterText.offsetParent
  ...
  // 获取所有子项处理为一维数组
  function getChildrenItem(arr, res) {
    if (!arr)
      return res
  ...
  // 判断父节点是否拖入其子节点中
  function isDragInChildNode(dragData, enterData) {
    if (dragData.itemId === enterData.itemId)
      return true
  ...
  // 处理拖拽排序前后的数据
  function changeData(parentData, dragData, enterData, positionValue) {
    ...
    while (stack.length > 0) {
      const cur = stack.pop() || []
      const ids = cur.map(item => item.itemId)
      const dragIndex = ids.indexOf(dragData.itemId)
      const enterIndex = ids.indexOf(enterData.itemId)
      // 同级情况
      if (dragIndex >= 0 && enterIndex >= 0) {
        if ((dragIndex - enterIndex === -1 && positionValue === 0))
          return
        if ((dragIndex - enterIndex === 1 && positionValue === 1))
          return
      }
      ...
      if (!addOk || !removeOk)
        cur.forEach(item => stack.push(item.children))
      else
        return res
    }
  }
